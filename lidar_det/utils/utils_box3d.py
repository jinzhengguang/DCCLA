...
def boxes_to_string(boxes, scores, jrdb_format=True):
    ...
    for box, score in zip(boxes, scores):
        if score > 1e-3:
            s += (
                f"Pedestrian 0 0 -1 0 -1 -1 -1 -1 {box[3]:.8f} {box[4]:.8f} {box[5]:.8f} "
                f"{box[0]:.8f} {box[1]:.8f} {box[2]:.8f} {box[6]:.8f} {score:.8f}\n"
            )
    s = s.strip("\n")
...
if __name__ == '__main__':
    ...
    # ious = my_iou_3d(box_3d_1, box_3d_2)
    # https://blog.csdn.net/qq_44523137/article/details/123259979
    # https://blog.csdn.net/SpriteNym/article/details/127965618

    paths = '/home/gjz/PycharmProjects/minkunet/bin/000000.txt'
    with open(paths, 'r') as f:
        pt = f.read()
    boxes, scores = string_to_boxes(pt)
    # temp = np.copy(boxes[1])
    # boxes[1][3] = boxes[1][3] - 0.8
    # ious = get_iou3d(temp.reshape(-1,7), boxes[1].reshape(-1,7))

    inds, inverse_map = nms_3d_dist(boxes, scores, l_ave=0.9, w_ave=0.5, nms_thresh=0.9)
    # temp = nms_3d_gpu(boxes[:2], scores[:2], iou_thresh=0.3)
    # temp = nms_3d_dist_gpu(boxes, scores, l_ave=0.9, w_ave=0.5, nms_thresh=0.4)
    print(boxes)

# 2023-04-14 Jinzheng Guang
