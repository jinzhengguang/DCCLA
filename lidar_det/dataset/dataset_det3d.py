...
class NuScenesDet3D(_DatasetBase):
    def __init__(self, *args, **kwargs):
        ...
        if nc > 0:
            ...
            self._inds_to_cls = np.array(inds_to_cls)
            self._dist_thresh = np.array(dist_thresh)
            self._ave_lwh = np.array(ave_lwh)
