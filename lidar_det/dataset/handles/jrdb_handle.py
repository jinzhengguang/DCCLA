...
class JRDBHandleDet3D:
    def __init__(self, data_dir, split, sequences=None, exclude_sequences=None):
        data_dir = os.path.abspath(os.path.expanduser(data_dir))
        data_dir = (
            os.path.join(data_dir, "train_dataset_with_activity")
            if split == "train" or split == "val"
            else os.path.join(data_dir, "test_dataset_without_labels")
        )

        self.data_dir = data_dir

        sequence_names = (
            os.listdir(os.path.join(data_dir, "pointclouds/lower_velodyne"))
